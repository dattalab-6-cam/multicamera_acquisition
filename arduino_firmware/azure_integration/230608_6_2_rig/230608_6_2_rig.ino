...
// HARDCODE ALL THE PINS
// TODO: These pins can be taken as input from jupyter
...
// Define the input GPIOs (for sync signal)
int num_input = 4;
...
// wait 100ms before doing anything to give the serial time to connect
...
// Flips the camera switch to trigger a frame acquisition
...
// a single loop, where we decide to check if the azure should trigger a pulse, 
//   or end a pulse trigger, based on the time
...
// a single loop, where we decide to trigger the basler top and bottom lights / cameras
//   or turn them off
...
// set the pins and start the serial connection
void setup()
{
  // set up IR pins
...
// wait for a serial connection, then start acquisition loop
void loop()
{
